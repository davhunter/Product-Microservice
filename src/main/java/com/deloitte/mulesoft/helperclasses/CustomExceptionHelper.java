...
public final class CustomExceptionHelper {
	...
	// Causes the class to initialize itself
	static {
		initialize();
	}
	 ...
	 * </p>
	 * 
	 * @see org.mule.config.ExceptionHelper#getErrorCode(Class)
	 * @param exception
	 ...
	 * JSON string property. Uses Apache's <code>StringEscapeUtils</code> to do
	 * the heavy lifting; the <code>StringEscapeUtils.escapeJson()</code> method
	 * doesn't exist in the version included in Mule, so
	 * <code>escapeEcmaScript()</code> was used instead.
	 * </p>
	 * 
	 * <p>
	 * One side effect of using <code>escapeEcmaScript()</code> instead of
	 * <code>escapeJson()</code> is that single quotes are mishandled; they are
	 * escaped, but shouldn't be. So this code fixes that mistake.
	 * </p>
	 * 
	 * @see org.apache.commons.lang3.StringEscapeUtils#escapeEcmaScript(String)
	 * 
	 * @param input
